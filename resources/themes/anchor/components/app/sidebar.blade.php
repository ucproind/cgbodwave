    ...
    <div :class="{ '-translate-x-full': !sidebarOpen }"
        ...
        <div class="flex flex-col justify-between w-full overflow-auto md:h-full h-svh pt-4 pb-2.5">
            <div class="relative flex flex-col">
                ...
                <div
                    ...
                    @php
                        ...
                        $isProjectPage = Request::is('projects/*') || Request::is('project/*');

                        if ($isAdmin) {
                            $projects = App\Models\CgbodProject::all();
                        } else {
                            $projects = $user->cgbodProjects ?? collect();
                        }
                    @endphp
                    ...
                    <x-app.sidebar-dropdown text="Projects" icon="phosphor-stack" id="projects_dropdown"
                        ...
                        @if ($projects->count() > 0)
                            @foreach ($projects as $project)
                                <x-app.sidebar-link href="#"
                                    icon="phosphor-cube" :active="Request::is('projects/' . $project->id) ||
                                        Request::is('project/' . $project->slug)">
                            ...
                            @endforeach
                        @else
                            <x-app.sidebar-link href="#" icon="phosphor-warning-circle"
                                onclick="event.preventDefault(); new FilamentNotification().title('No projects found').send()">
                                No Projects
                            </x-app.sidebar-link>
                        @endif
                    ...
                    <x-app.sidebar-link
                        ...
                        icon="phosphor-pencil-line" active="false">Stories</x-app.sidebar-link>

                    @if ($isAdmin)
                        <x-app.sidebar-link
                            onclick="event.preventDefault(); new FilamentNotification().title('Modify this button inside of sidebar.blade.php').send()"
                            icon="phosphor-users" active="false">Users</x-app.sidebar-link>
                    @endif
                </div>
            </div>
            ...
            <div class="relative px-2.5 space-y-1.5 text-zinc-700 dark:text-zinc-400">
                ...
                <x-app.sidebar-link href="https://devdojo.com/wave/docs" target="_blank"
                    icon="phosphor-book-bookmark-duotone" active="false">Documentation</x-app.sidebar-link>
                <x-app.sidebar-link href="https://devdojo.com/questions" target="_blank" icon="phosphor-chat-duotone"
                    active="false">Questions</x-app.sidebar-link>
